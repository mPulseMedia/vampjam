...
(async () => {
  ...
  ok('and told so plainly, not left waiting',  /not on any recording/.test(no.why || ''), no.why);
  ...
  // and a recording nobody was added to belongs to nobody but the administrator
  ACCESS.sessions['2026_07_31_sound_union.html'] = { allow: [] };
  const inn2 = await worker('enter', {}, { phone: '650-555-0000' });
  ok('a recording he is not on stays off his list',
  ...
  ok('this recording is restricted to those two',
     w.sessions['2026_08_14_sound_union.html'].allow.length === 2, JSON.stringify(w.sessions));
  ...
  // The signed-out case belongs to gate_all now: a signed-out person never
  // reaches a recording at all — they are turned around at the door. There is
  // no half-state left where the box is visible to someone who cannot use it.
  ...
  await ctx2.route('**/*', async (r) => {
    ...
    if (u.indexOf('vampjam-auth') >= 0) return r.fulfill({ status: 200,
      contentType: 'application/json', body: '{"ok":true,"signed_in":true,"admin":true,"allow":"*"}' });
    if (u.startsWith('https://vampsf.com/')) {
      const rel = u.replace('https://vampsf.com/', '').split('?')[0] || 'index.html';
      const p3 = path.join(DIR, rel);
      if (fs.existsSync(p3)) {
  ...
  ok('a blocked access.json no longer stops the box',
     bl.canType === true, JSON.stringify(bl));
  await blocked.close();
  await ctx2.close();
  ...
  // The typo-recovery machinery is gone with the redesign: the first person to
  // SIGN IN is the administrator, so there is no separate bootstrap to rescue.
  // What replaced it is asserted in gate_all_test.
