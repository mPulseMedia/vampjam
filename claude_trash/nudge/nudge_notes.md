...
The experiment's own record, lifted out of the host's spec so it travels with the
code. Every entry here is one that touched `lab.html` (now `nudge/nudge.html`),
from the first sketch at 177 to the split at 279. The numbering is the HOST's
build number and the gaps in it are the host's own builds; they are kept rather
than renumbered, because the numbers are what the prompt log, the commit messages
and eighteen months of conversation refer to.

Two entry formats appear, because the host's spec changed shape partway through:
the early ones read `- (log N name — where)` and the later ones
`- N name · bN · where`.

Read it backwards from the end for the current state of anything; read forward
from 177 for how a dot on a card became a nudge detector with three rooms.
